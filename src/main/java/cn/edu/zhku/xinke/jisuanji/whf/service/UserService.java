...
public class UserService {
	...
	public ModelAttribute register(User user) {
		...
		if (!Pattern.matches("^[a-zA-Z_0-9]+@[a-zA-Z_0-9]+\\.[a-zA-Z_0-9]+$",
				user.getEmail())) {
			// 邮箱格式有误
	...
	public ModelAttribute login(String username, String password,
			HttpSession session) {
	...
	public ModelAttribute update(User user, HttpSession session) {
		ModelAttribute ma = new ModelAttribute("forward:info.jsp");
		User curUser = (User) session.getAttribute(User.CURRENT_USER);
		if (curUser == null) {
			ma.setDestination("forward:message.jsp");
		...
		User userGet = null;
		userGet = userDao.getByName(user.getName());
		if (userGet != null && userGet.getId() != user.getId()) {
			// 用户名重复
			ma.setAttribute("message", "用户名已被占用");
			ma.setDestination("forward:register.jsp");
			return ma;
		...
		int res = userDao.update(curUser);
		if(res > 0){
			ma.setAttribute("message","修改成功");
		}else {
			ma.setAttribute("message", "修改失败");
		}
		return ma;
